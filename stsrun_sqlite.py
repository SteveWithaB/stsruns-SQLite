...
def RunInfo(rundata):
    ...
    try:
        ...
        killed_by = ""
    stmt = "INSERT INTO runinfo values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
...
def MasterDeck(play_id, timestamp, master_deck):
    ...
    stmt = "INSERT INTO masterdecks values (?, ?, ?)"
...
def FloorInfo(rundata):
    ...
    stmt = "INSERT INTO floorinfo values (?,?,?,?,?,?,?,?,?)"
...
def ScoreDetail(play_id, timestamp, score_breakdown):
    ...
    stmt = "INSERT INTO scoredetail values (?,?,?,?)"
...
def CardRewards(play_id, timestamp, card_choices):
    ...
    stmt = "INSERT INTO cardrewards values (?,?,?,?,?)"
...
def Relics(play_id, timestamp, relic_stats):
    ...
    stmt = "INSERT INTO relics values (?,?,?,?,?,?,?)"
...
def RelicsMissed(rundata):
    ...
    if len(relics_missed) > 0:
        stmt = "INSERT INTO relicsmissed values (?, ?, ?, ?, ?)"
...
def RestSites(play_id, timestamp, campfire_choices):
    ...
    stmt = "INSERT INTO restsites values (?, ?, ?, ?, ?)"
...
def BossRelics(play_id,timestamp,boss_relics):
    ...
    for bossrelic in boss_relics:
        ...
        f=33
    stmt = "INSERT INTO bossrelics values (?,?,?,?,?)"
    InsertData(stmt,relic_list)
...
def Combats(play_id, timestamp, damage_taken):
    ...
    stmt = "INSERT INTO combats values (?, ?, ?, ?, ?, ?)"
...
def LessonLearned(play_id, timestamp, lesson_learned):
    ...
    if len(learned_list) > 0:
        stmt = "INSERT INTO lessonlearned values (?, ?, ?, ?)"
...
def PotionDetails(rundata):
    ...
    if len(potion_list) > 0:
        stmt = "INSERT INTO potiondetail values (?,?,?,?,?)"
...
def NeowDetails(rundata):
    ...
    stmt = "INSERT INTO neowdetails values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
...
def Shops(rundata):
    ...
    stmt = "INSERT INTO shoppurchases values (?,?,?,?,?)"
    ...
    stmt = "INSERT INTO shopskipped values (?,?,?,?,?)"
...
# Only needed if this is a first run and you need to load the s_object table
def LoadObjects():
    url = "https://raw.githubusercontent.com/OceanUwU/slaytabase/refs/heads/main/docs/slay%20the%20spire/data.json"
    response = urllib.request.urlopen(url)
    sObject = json.loads(response.read())
    cards = sObject["cards"]
    insvalues = []
    for card in cards:
        if card["mod"] == "Slay the Spire":
            insvalues.append(("Card",card["id"],card["name"],card["mod"],card["rarity"],card["type"],card["cost"],card["color"]))
    relics = sObject["relics"]
    for relic in relics:
        if relic["mod"] == "Slay the Spire":
            insvalues.append(("Relic",relic["id"],relic["name"],relic["mod"],relic["tier"],'','',relic["pool"]))
    potions = sObject["potions"]
    for potion in potions:
        if potion["mod"] == "Slay the Spire":
            if "color" in potion.keys():
                potcolor = potion["color"]
            else:
                potcolor = ''
            insvalues.append(("Potion",potion["id"],potion["name"],potion["mod"],potion["rarity"],'','',potcolor))
    stmt = "INSERT INTO s_object VALUES (?,?,?,?,?,?,?,?)"
    InsertData(stmt,insvalues)
